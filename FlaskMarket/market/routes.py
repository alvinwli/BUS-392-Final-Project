    ...
    if request.method == "POST":
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"You sold {s_item_object.name}", category='success')
            else:
                flash(f"There was a problem selling {s_item_object.name}", category='danger')
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(f"Account has been created. You are now logged in as {user_to_create.username}", category='success')
        return redirect(url_for('market_page'))
    if form.errors != {}: 
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash('Your username and password do not match. Please try again', category='danger')
